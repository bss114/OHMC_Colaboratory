...
Note: This template is for simple cross sectional analyses, but contains the code for importing and processing any study design. See also the inclusion of a parallel function for linear mixed effects models for more complex study designs including repeated measures designs.

## Experimental Methods

Information on how samples were prepared and sequenced here. For Example if using standard lab workflow:

Genomic DNA was extracted from samples using a DNeasy 96 PowerSoil Pro QIAcube HT Kit with liquid handling performed by a Integra mini-96 1250µL multichannel and a QIAcube HTP extraction platform. Samples were quantified by spectrophotometry (Nanodrop One), and 30ng was used as the input to a 1/5th scale Illumina DNA Preparation reaction with all other paramters of library preparation according to the manufacturer's instructions. Samples were indexed with unique dual indexes based on the Illumina Tagmentation Set A-D. 1 µL of each sample was pooled for quality control by sequencing on an Illumina iSeq100 2x150 run. Samples were re-pooled to normalize sequencing depth before being run on Illumina NovaSeq X 2x150 25B lanes (Novogene USA). All liquid handling steps of library preparation, normalization, and quality control were performed using Integra mini-96s and Opentrons OT-2s with designated pre- and post-PCR units inside flow hoods (Airclean PCR workstations). 

Samples obtained XX,XXX,XXX ± X,XXX,XXX reads (mean±sd). Negative controls obtained less than X reads. Samples were processed by first using FastP to detect and remove adapters, remove polyG runs, and perform sliding window quality filtering (Chen et al. Bioinformatics 2018). Next reads mapping to host DNA were removed using Kneaddata using provided reference assemblies (McIver Bioinformatics 2018). Sample normalization factors were determined using MicrobeCensus (Nayfach et al. Genome Biology 2015). Taxonomic abundances were determined using MetaPhlAn v4.0.6 including viruses and unclassified estimation. Gene family abundances and pathway abundances were determined using HUMAnN v3.9 against the uniref90 database (Beghini et al. eLife 2021). Taxonomic and Pathway abundances were normalized as the log2 of the reported abundance. Gene family abundances were normalized as RPKG (read per kilobase per genome equivalent as derived from MicrobeCensus). Alpha diversity metrics were determined using Vegan v2.6-10. Principal coordinates analysis was performed as implemented in Ape v5.8-1. Where relevant longitudinal data was analyzed in linear mixed effects models using log-transformed data and lmerTest v1.1-37. PerMANOVA was implemented using the adonis2 function of Vegan. All P-values were corrected with Benjamini Hochberg False Discovery Rate unless otherwise indicated.
...
```{r}
QC_data<-
list.files(paste0(indir, "/fastp/"), pattern="\\json$", full.names=TRUE) %>%
  grep(prefix, ., value=TRUE) %>%
  lapply(., function(x){
    tmp<-jsonlite::fromJSON(txt=x)
    tibble(
    SampleID=basename(x) %>% gsub("\\.json","", .),
    Percent_Duplication=tmp$duplication$rate*100,
    PreQC_Reads=tmp$summary$before_filtering$total_reads,
    PreQC_Gb=tmp$summary$before_filtering$total_bases/1e9,
    PostQC_Reads=tmp$summary$after_filtering$total_reads,
    PostQC_Gb=tmp$summary$after_filtering$total_bases/1e9
    )
  }) %>%
  bind_rows(.)
  
interactive_table(QC_data)
write_tsv(QC_data, "processed_data/QC_data.tsv")

QC_data %>%
  pivot_longer(!SampleID, names_to = "Metric") %>%
  group_by(Metric) %>%
  summarize_at("value", lst(mean,median,sd,min,max)) %>%
  interactive_table()
```
...
```{r}
PathDists<-vegan::vegdist(t(Pathway_filled), method = "bray")
PathPC<-ape::pcoa(PathDist)
...
PathPC$vectors %>%
  ...
  geom_point(shape=21) +
  ggrepel::geom_text_repel(size=3) +
  xlab(paste0("PC1:", round(PathPC$values$Rel_corr_eig[1]*100, 2), "%")) +
  ylab(paste0("PC2:", round(PathPC$values$Rel_corr_eig[2]*100, 2), "%"))

ggsave("figures/PATHWAY_brayPCoA.pdf", height=7.5, width=10, useDingbats=F)

adonis2(PathDists~Group, data=metadata[match(labels(PathDists), metadata$SampleID),]) %>%
  print()
```
...
GenePC$vectors %>%
  ...
  geom_point(shape=21) +
  ggrepel::geom_text_repel(size=3) +
  xlab(paste0("PC1:", round(GenePC$values$Rel_corr_eig[1]*100, 2), "%")) +
  ylab(paste0("PC2:", round(GenePC$values$Rel_corr_eig[2]*100, 2), "%"))

ggsave("figures/GENE_brayPCoA.pdf", height=7.5, width=10, useDingbats=F)

adonis2(GeneDists~Group, data=metadata[match(labels(GeneDists), metadata$SampleID),]) %>%
  print()
```
...
```{r, eval=TRUE}
NSamps<-length(unique(metadata$SampleID))*0.2 # 0.2 = feature must occur in at least 20% of samples to do statistical analysis
ToFilter<-GeneAbundances_filled %>% group_by(GeneFamily) %>% summarize_at("RPKG", list(Keep=function(x) sum(x>0)>=NSamps)) %>% filter(Keep) # only test features where they occur it at least 20% of samples as will not be informative and gene-level data can be very sparse.


Gene_differential<-
  GeneAbundances_filled %>%
  #filter(GeneFamily %in% sample(unique(GeneFamily), 1000)) %>% # for testing on small subset
  filter(GeneFamily %in% ToFilter$GeneFamily) %>%
  left_join(metadata) %>%
  DoParallel_ttest(., 10, "GeneFamily","Group","log2RPKG")
